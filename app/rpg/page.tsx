...
export default function RpgPage() {
  ...
  const handleShare = async (text: string) => {
    const shareData = {
        ...
        url: window.location.origin // Sitenin o anki adresi
    };

    try {
        if (navigator.share) {
            // Mobilde native paylaşım menüsünü açar (Instagram, WP vs.)
            await navigator.share(shareData);
        } else {
            // Masaüstünde panoya kopyalar
  ...
  const startGame = async () => {
    ...
    } catch (err) { alert("BAĞLANTI KOPTU"); } finally { setLoading(false); }
  };
  ...
  const makeChoice = async (choice: string) => {
    ...
    } catch (err) { alert("SİMÜLASYON HATASI"); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen text-cyan-400 font-['Fira_Code'] flex flex-col items-center p-2 sm:p-4 cyber-grid relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl border-b border-cyan-800 pb-2 mb-4 flex justify-between items-center sticky top-0 bg-black/95 z-50 pt-2 backdrop-blur-md shadow-[0_10px_20px_rgba(0,0,0,0.8)]">
        <h1 className="text-lg sm:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-600 truncate mr-2 tracking-widest drop-shadow-[0_0_5px_rgba(0,255,255,0.5)]">
          NO_REGRET_MACHINE
        </h1>
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] text-pink-500 animate-pulse hidden xs:block font-bold ml-1">● ON-AIR</p>
            </div>
        </div>
      </div>
      ...
      {/* ADIM 1: GİRİŞ FORMU */}
      {step === 1 && (
        <div className="w-full max-w-lg border border-cyan-500/50 bg-black/80 p-6 sm:p-8 mt-10 shadow-[0_0_30px_rgba(6,182,212,0.2)] animate-in fade-in zoom-in duration-500 relative group">
          ...
          <div className="space-y-5">
            <div>
               ...
               <input name="name" value={formData.name} onChange={handleInput} className="w-full bg-gray-900/50 border border-gray-700 text-cyan-300 p-3 outline-none focus:border-cyan-400 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] transition-all" placeholder="Giriş yap..." />
            </div>
            
            <div>
               <label className="text-[10px] text-cyan-600 mb-1 block uppercase tracking-widest">Yıldız Konumu</label>
               <select name="sign" value={formData.sign} onChange={handleInput} className="w-full bg-gray-900/50 border border-gray-700 text-cyan-300 p-3 outline-none focus:border-cyan-400">
            ...
            <div>
               ...
               <textarea name="regret" value={formData.regret} onChange={handleInput} rows={3} className="w-full bg-gray-900/50 border border-gray-700 text-pink-300 p-3 outline-none focus:border-pink-500 focus:shadow-[0_0_10px_rgba(236,72,153,0.3)] resize-none" placeholder="Veri giriniz..." />
            </div>
